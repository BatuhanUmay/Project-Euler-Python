NAMES = [  # 10 strings per line, except final line
    ...
    "JENETTE", "GABRIELE", "EVELYNE", "ELENORA", "CLEMENTINA", "ALEJANDRINA", "ZULEMA", "VIOLETTE", "VANNESSA",
    "THRESA",
...
for num, letter in enumerate(letters, 1):
    dictLetters[letter] = num


def calculate(name, position):
    letterScore = 0
    for letter in name:
        letterScore += dictLetters.get(letter)
...
for i in range(len(NAMES)):
    score += calculate(NAMES[i], NAMES.index(NAMES[i]) + 1)
print(score)

#######################################################################

# def compute():
#     ans = sum((value) * (ord(char) - ord('A') + 1)
#         for value, name in enumerate(sorted(NAMES), 1)
#         for char in name
#     )
#     return ans
#
