...
def left_right():
    ...
    for row in range(0, len(GRID)):
        for column in range(0, len(GRID[0]) - CONSECUTIVE + 1):
            product = 1
            for k in range(4):
                product *= GRID[row][column + k]
...
def up_down():
    largest_product = 0
    for row in range(0, len(GRID) - CONSECUTIVE + 1):
...
def diagonal_right_left():
    ...
    for row in range(0, len(GRID) - CONSECUTIVE + 1):
        for column in range(0, len(GRID[0]) - CONSECUTIVE + 1):
            product = 1
            for k in range(4):
                product *= GRID[row + k][len(GRID[0]) - 1 - (column + k)]
                if product > largest_product:
                    largest_product = product
    return largest_product
